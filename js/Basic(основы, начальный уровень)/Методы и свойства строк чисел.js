...
console.log(fruit.indexOf("f"));        //вывести в консоль номер или индекс с какой позиции (число) начинается символ f
...
console.log(Math.round(num));           //вывести в консоль результат округления num ответ целлые числа

const test = "12.8px";                  //создание фиксир. переменно test присвоения текста для тестирования
console.log(parseInt(test));            //вывести в консоль результат преобразования строки в число (целое)
console.log(parseFloat(test));          //вывести в консоль результат преобразования строки в число (дробное)